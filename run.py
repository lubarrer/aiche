...
def axopt(config):
    def law_of_cosines(len1, len2, angle):
        return np.sqrt(
            len1 ** 2 + len2 ** 2 - 2 * len1 * len2 * np.cos(np.radians(angle))
        )
    ...
    def redeploy_module(
        mod_size, distance
    ):  # move a module from one well to another well
        if mod_size == "small":
            return -(
                500 * 3 * distance
                + (unit_size("small", 1, 0.33, 0.33, 0.33)) * 30 / 292
            )
        elif mod_size == "medium":
            return -(
                2500 * 3 * distance
                + (unit_size("medium", 1, 0.33, 0.33, 0.33)) * 30 / 292
            )
        elif mod_size == "large":
            return -(
                5000 * 3 * distance
                + (unit_size("large", 1, 0.33, 0.33, 0.33)) * 30 / 292
            )
    ...
    def init_deploy():
        ...
        for id in module_id:
            ...
            if current_well in current_wells:
                ...
                revenue, current_well = deploy_unit(
                    size, 0, 0, max_prod[-i], deduct=deduct
                )
...
if __name__ == "main":
    ...
    config = {"num_small": 1, "num_medium": 1, "num_large": 1}

    best_npv = axopt(config)
    print(best_npv)
